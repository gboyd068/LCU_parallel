...
def convert_tableau_to_symplectic(tableau):
    binmats = list(map(BinaryMatrix, map(lambda x: x.astype(int), tableau.to_numpy())))[
        :-2
    ]
...
def convert_symplectic_to_tableau(sym_mat):
    ...
    return stim.Tableau.from_numpy(
        np.block(
            [[sym_mat[:n, :n], sym_mat[:n, n:]], [sym_mat[n:, :n], sym_mat[n:, n:]]]
        )
    )
...
def cnot_sym(M):
    ...
    if not np.linalg.det(M):
        raise ValueError(
            "Input matrix cannot be converted into a CNOT circuit as it is not invertible."
        )
    return BinaryMatrix(
        np.block(
            [
                [M, BinaryMatrix.Zeros((n, n))],
                [BinaryMatrix.Zeros((n, n)), np.linalg.inv(M.transpose())],
            ]
        )
    )
...
def partial_phase_sym(diagonal):
    ...
    return BinaryMatrix(
        np.block(
            [
                [BinaryMatrix.Identity(n), BinaryMatrix.Zeros((n, n))],
                [diagonal, BinaryMatrix.Identity(n)],
            ]
        )
    )
...
def clifford_canonical_form(tableau, test=False):
    ...
    if test:
        assert np.linalg.det(get_blocks(mat)[1])
    ...
    if test:
        assert (get_blocks(mat)[1] == BinaryMatrix.Identity(n_qubits)).all()
    ...
    if test:
        assert (get_blocks(mat)[3] == M @ M.transpose()).all()
    ...
    if test:
        assert (get_blocks(mat)[1] == Ninv).all()
        assert (get_blocks(mat)[3] == Ninv).all()
    ...
    if test:
        assert (get_blocks(mat)[1] == BinaryMatrix.Identity(n_qubits)).all()
        assert (get_blocks(mat)[3] == BinaryMatrix.Identity(n_qubits)).all()
    ...
    if test:
        assert (get_blocks(mat)[1] == BinaryMatrix.Identity(n_qubits)).all()
        assert (get_blocks(mat)[3] == BinaryMatrix.Zeros((n_qubits, n_qubits))).all()
    ...
    if test:
        assert (get_blocks(mat)[0] == BinaryMatrix.Identity(n_qubits)).all()
        assert (get_blocks(mat)[1] == BinaryMatrix.Zeros((n_qubits, n_qubits))).all()
        assert (get_blocks(mat)[3] == BinaryMatrix.Identity(n_qubits)).all()
    ...
    if test:
        assert (get_blocks(mat)[2] == MTinv @ MTinv.transpose()).all()
    ...
    if test:
        assert (get_blocks(mat)[0] == M).all()
        assert (get_blocks(mat)[1] == BinaryMatrix.Zeros((n_qubits, n_qubits))).all()
        assert (get_blocks(mat)[2] == MTinv).all()
        assert (get_blocks(mat)[3] == MTinv).all()
    ...
    if test:
        assert (get_blocks(mat)[0] == M).all()
        assert (get_blocks(mat)[1] == BinaryMatrix.Zeros((n_qubits, n_qubits))).all()
        assert (get_blocks(mat)[2] == BinaryMatrix.Zeros((n_qubits, n_qubits))).all()
        assert (get_blocks(mat)[3] == MTinv).all()
    ...
    # check we ended up with the identity matrix
    if test:
        assert (mat == BinaryMatrix.Identity(2 * n_qubits)).all()

    # get values to return

    # combined cnots layer matrix (eq 29)
    L = (
        np.linalg.inv(Q.transpose())
        @ np.linalg.inv(Ninv.transpose())
        @ MTinv.transpose()
    )
    M = np.linalg.inv(MTinv.transpose())
    ...
    # return in order of application
    return [CL, Pall, CM, P2, Hall, Pall, CN, P1, Hmat]
